...
import './index.scss'
import { setQuestionList, setAnswerId } from '../../store/modules/question'
...
export default function Index() {
  ...
  const getData = async () => {
    ...
    console.log(data);
    dispatch(setQuestionList(data.questions))  // 存仓库给结果页面使用
  ...
  const nextTopic = () => {
    ...
    if (num === ques.length) { // 最后一题
      ...
      if (selectedAnswer) { // 有选择答案
        dispatch(setAnswerId(selectedAnswer.topic_answer_id))
      }
      setNum(num + 1)
    }
  }
